...
export class JSONStorageEngine<Schema extends z.ZodObject> extends StorageEngine<Schema> {
    ...
    write: DSEngine<Schema>["write"] = async (data) => {
        ...
        try {
            await tempfile.write(JSON.stringify(Object.fromEntries(data), null, 2));
        } catch (error: any) {
    ...
    read: DSEngine<Schema>["read"] = async () => {
        ...
        try {
            const rawdata = JSON.parse(await file.readFile({ encoding: "utf-8" }));
